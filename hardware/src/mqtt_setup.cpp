...
String decryptData(String encryptText) {
  ...
  aesLib.decrypt64((char *)encryptText.c_str(), msgLen, (byte *)clearText,
                   aes_key, sizeof(aes_key), temp_iv);
...
void callback(char *topic, byte *payload, unsigned int length) {
  ...
  // Nếu có cmd_enc_value → giải mã 1 cục rồi parse lại
  if (!doc["cmd_enc_value"].isNull()) {
    const char *encValue = doc["cmd_enc_value"];
    String decrypted = decryptData(String(encValue));
    decrypted.trim();
    ...
    doc.clear();
    DeserializationError decErr = deserializeJson(doc, decrypted);
    if (decErr) {
  ...
  // Từ đây doc chứa JSON gốc: {"action":"pump_on","roomId":101}
  ...
  if (!doc["roomId"].isNull()) {
    ...
    if (targetRoomId != currentRoomId) {
      Serial.printf("\n[CTRL] Ignored: targetRoomId %d != local %d\n",
                    targetRoomId, currentRoomId);
      return;
    }
  }
  // Nếu không có roomId → broadcast → tất cả MCU đều thực thi
...
String encryptData(String plainText) {
  ...
  aesLib.encrypt64((byte *)plainText.c_str(), msgLen, (char *)cipher64, aes_key,
                   sizeof(aes_key), temp_iv);
    ...
    if (WiFi.status() == WL_CONNECTED) {
      if (!client.connected()) {
        if (String(mqtt_server) != "NO_IP") {
          if (xMqttMutex != NULL &&
              xSemaphoreTake(xMqttMutex, portMAX_DELAY) == pdPASS) {
